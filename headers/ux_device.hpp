...
public:
	...
	surface_area_t(const surface_area_t &other) :
			context(other.context), fnEvents(other.fnEvents), display_list_storage(
					other.display_list_storage) {
	...
	template<typename T> surface_area_t& operator<<(const T &data) {
		...
		if constexpr (std::is_base_of<listener_t<T>, T>::value) {
			...
			if constexpr (std::is_base_of<visitor_display_context_memory_t, T>::value)
			...
			if constexpr (std::is_base_of<
					emit_cairo_relative_coordinate_abstract_t, T>::value) {
			...
			if constexpr (std::is_base_of<emit_cairo_coordinate_abstract_t, T>::value) {
			}
...
UX_DECLARE_STREAM_INTERFACE(std::string_view)
	 ...
	 */
public:
	template<typename T> void in(const T &obj) {
		operator<<(obj);
	}
...
private:
	...
	void open_window(const coordinate_list_t &coord,
			const std::string &sWindowTitle, const painter_brush_t &background,
			const event_handler_t &dispatch_events);
...
#define UX_DISPLAY_LIST_CLEAR DL_readwrite.clear(std::memory_order_release)
	...
	std::unordered_map<indirect_index_display_unit_t,
			std::shared_ptr<display_unit_t>> mapped_objects = { };
