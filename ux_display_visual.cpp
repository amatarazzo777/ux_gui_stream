...
\date 9/7/20
\version 1.0
\brief The display_visual_t object is a base class for objects that
can be drawn on the surface. The implementation provides a communication
mechanism between a display unit and the display_context_t. Display
units that inherit the object must also use the pipeline_memory_t
type. The system depends on the display unit to call the set_ink()
function as the rectangle is needed to determine where and how big
...
\brief used to determine if multiple requests are issued for the
same object. in which the system will develop an image of the pipeline
and use the image in the future rather than calling the pipeline
function again. This is useful for text that may move or require repaint
for other purposes. However this function is not only for text. All objects
that implement a pipeline are provided with the functionality,
*/
